...
#    def plot_residual(self, plot_type = 'Re'):
#        # plot_fitting residual
#        if plot_type == 'Re':
#            pass
#            
#        elif plot_type == 'Im':
...
def Simple_run(entry, rbf_type = 'Gaussian', data_used = 'Combined Re-Im Data', induct_used = 1,
    ...
    """
        ...
        Args:
            ...
            data_used: Part of EIS data used for regularization
    ...
    # define the bounds of optimization
    ...
    # Step 2: Conduct the Ridge regularization
    # Step 2.1: select the part of impedance used fo simple run
    if data_used == 'Combined Re-Im Data':
        ...
        x = gf.cvxpy_solve_qp(H_combined,c_combined)
        
        # Step 2.2: Carry out ridge regression prepare for HMC sampler
        ...
        # here we add the matrix column for resistence
        N_RL = 1
        entry.A_re = np.zeros((N_freqs, N_taus+N_RL))
        entry.A_re[:, N_RL:] = entry.A_re_temp
        entry.A_re[:,0] = 1
...
def Bayesian_run(entry, rbf_type = 'Gaussian', data_used = 'Combined Re-Im Data', induct_used = 1,
    ...
    """
        ...
        Args:
            ...
            **kwargs: Other arguments that are applicable for Simple_run
    """
    ...
    # Use cholesky instead of direct inverse 
    L_Sigma_inv = np.linalg.cholesky(entry.Sigma_inv)
...
def BHT_run(entry, rbf_type = 'Gaussian', der_used = '1st order',
    ...
    # try until no error occur for for the HT_single_est
    while True:
        try:
    ...
    # Step 5 shows the band and the hilbert fitting in the real part and the imag part
    # 1. real data
...
if __name__ == '__main__':
    ...
    """
    # Import EIS file
    object_A = EIS_object.from_file('ema_test.txt')
    # Choose the type of run 
    Simple_run(object_A, data_used = 'Combined Re-Im Data', induct_used = 1, der_used = '1st order', lambda_value = 1E-3)
